...
do
	...
	local function NextOp (ops, count, name, ...)
		if count > 0 then
			...
			assert(IsCallable(op), "Invalid operation")

			return op, NextOp(ops, count - 1, ...)
		end
	end
	...
	function GetMaxStringW( Widgets )
		local w = 0
		for i, widget in ipairs( Widgets ) do
			local widgetW = StringGetW( widget, widget:GetString() )
			w = widgetW > w and widgetW or w
		end
		return w
