...
local function GetEntry (entry, A)
	return CallOrGet(entry[1], A:GetFont()), unpack(entry, 2, entry.n)
end
...
class.Define("ArrayView", function(ArrayView)
	...
	ArrayView.Iter = iterators.CachedCallback(function()
		...
		-- Body --
		return function(_, i)
			return i + 1, GetEntry(self[_array][i + 1], self)
		end,

		-- Done --
		function(_, i)
			return i + 1 >= final or i >= #self[_array]
		end,
