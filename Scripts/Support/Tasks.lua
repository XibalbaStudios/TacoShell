...
function PersistUntil (update, quit)
	...
	local age, diff = 0, GetTimeLapseFunc("tasks")
...
local function SetupTimer (timer)
	...
	if type(timer) == "number" then
		...
		diff, timer = GetTimeLapseFunc("tasks"), New("Timer")
...
function WithTimeline (timeline, func, quit, time, is_absolute)
	...
	if not timeline then
		diff, timeline = GetTimeLapseFunc("tasks"), New("Timeline")
	end
