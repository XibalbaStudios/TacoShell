...
-- Cached routines --
local GetTracebackFunc_
local Identity_
local NoOp_
local StoreTraceback_
...
do
	...
	-- resource is still released, a traceback is stored, and the error propagated.<br><br>
	...
	-- @param extra_levels Number of levels (0 or more) above the caller to report any
	-- error.
	...
	function WithResource (acquire, use, release, extra_levels, ...)
        assert(IsInteger(extra_levels) and extra_levels >= 0, "Invalid extra levels")
...
do
	...
	function StoreTraceback (...)
		LastTraceback = (TracebackFunc ~= nil and TracebackFunc or NoOp_)(...)
	end
end
...
Identity_ = Identity
NoOp_ = NoOp
StoreTraceback_ = StoreTraceback